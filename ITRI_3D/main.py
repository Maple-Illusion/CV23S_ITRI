...
if __name__ == '__main__':
    ...
    img = cv2.imread(img_root,1)
    h, w, c = img.shape
    
    cam_params = cam.read_camera_info(cam_root)
    print(cam_params[0])
    print(cam_params[1][0])
    if args.cam_type == 'fisheye':
        # new = np.zeros_like(img)
        Knew = cam_params[0].copy()
        Knew[(0,1), (0,1)] = 1 * Knew[(0,1), (0,1)]
        new = cv2.fisheye.undistortImage(img,np.array(cam_params[0]),D=np.array(cam_params[1][0]) ,Knew=Knew)
        # print('done')
    # print(img.shape)
    cv2.imshow('good',new)
